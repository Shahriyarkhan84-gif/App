...
| Agency portal | `agency_portal(p_agency?)` → `{ agency{id,name,code,status}, role, stats{hosts,verified,live_now,in_review,earnings_lifetime}, hosts[], applications[] }` (money and applicant phones only for admins/managers); `regenerate_agency_code(p_agency)` (agency admin/manager or owner) |
