...
insert into public.agencies (name, code) values ('Lahore Stars', 'AG-LHR001');

-- Clients can't submit results or write applications directly.
select set_config('request.jwt.claims', '{"sub":"ha_ok"}', false);
set role authenticated;
select tests.fails($$select public.internal_submit_host_application('ha_ok', 'Ayesha Khan', '+923001234567', '4567', 'AG-LHR001', 'Approved', 'Approved', 90, true, true, 25, 'r1', 'r2')$$, '%permission denied%', 'client submits result');
...
select tests.ok((public.internal_submit_host_application('ha_ok', 'Ayesha Khan', '+923001234567', '4567', 'ag-lhr001', 'Approved', 'Approved', 90, true, true, 25, 'r1', 'r2') ->> 'status') = 'approved', 'clean application approved');
-- Mismatched CNIC number → a person reviews it.
select tests.ok((public.internal_submit_host_application('ha_review', 'Bilal Ahmed', '+923111234567', '1111', 'AG-LHR001', 'Approved', 'Approved', 88, false, true, 30, 'r3', 'r4') ->> 'status') = 'in_review', 'cnic mismatch goes to review');
-- Face doesn't match → declined.
select tests.ok((public.internal_submit_host_application('ha_bad', 'Sara Ali', '+923211234567', '2222', 'AG-LHR001', 'Approved', 'Declined', 12, true, true, 22, 'r5', 'r6') ->> 'status') = 'declined', 'face mismatch declined');
-- Under 18 → declined.
select tests.ok((public.internal_submit_host_application('ha_kid', 'Young One', '+923331234567', '3333', 'AG-LHR001', 'Approved', 'Approved', 90, true, true, 16, 'r7', 'r8') ->> 'status') = 'declined', 'minor declined');
select tests.fails($$select public.internal_submit_host_application('ha_bad', 'Sara Ali', '+923211234567', '2222', 'AG-NOPE00', 'Approved', 'Approved', 90, true, true, 22, 'r9', 'r10')$$, '%invalid_agency_code%', 'unknown agency code');
select tests.fails($$select public.internal_submit_host_application('ha_ok', 'Ayesha Khan', '+923001234567', '4567', 'AG-LHR001', 'Approved', 'Approved', 90, true, true, 25, 'r11', 'r12')$$, '%already_verified%', 'no re-application once approved');
...
select tests.fails($$select public.internal_submit_host_application('ha_bad', 'Sara Ali', '0321', '2222', 'AG-LHR001', 'Approved', 'Approved', 90, true, true, 22, 'r13', 'r14')$$, '%host_applications_phone_check%', 'phone format enforced');
reset role;

select tests.ok((select verification_status = 'approved' and agency_id = (select id from public.agencies where code = 'AG-LHR001') from public.hosts where user_id = 'ha_ok'), 'approved host verified and linked to agency');
